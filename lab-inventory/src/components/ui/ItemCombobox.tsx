...
export function ItemCombobox({
  ...
  useEffect(() => {
    ...
    function handle(e: MouseEvent) {
      const portal = document.getElementById('item-combobox-portal')
        ...
        {loading ? (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className="text-muted-foreground ml-2 text-xs">{item.category} · {item.unit}</span>
            </div>
          </li>
        ))}
      </ul>
    ...
    <div className="relative">
      ...
      >
        <span className={selected ? 'text-foreground truncate text-left' : 'text-muted-foreground'}>
          {loading ? 'Chargement…' : selected ? `${selected.name} (${selected.unit})` : placeholder}
        </span>
